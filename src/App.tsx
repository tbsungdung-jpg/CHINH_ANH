...
type Outfit = 'Áo sơ mi' | 'Áo Vest' | 'Áo Dài' | 'Áo Thun' | 'Áo Đoàn' | 'Đồ Công Sở';
type Quality = '2K' | '4K' | '8K';
type VestStyle = 'Cổ điển' | 'Hiện đại' | 'Slim-fit' | 'Double-breasted';
type VestColor = 'Đen' | 'Xanh Navy' | 'Xám' | 'Trắng' | 'Đỏ đô' | 'Be';
type TieColor = 'Không' | 'Đỏ' | 'Xanh' | 'Đen' | 'Vàng' | 'Họa tiết';
type AoDaiColor = 'Trắng' | 'Đỏ' | 'Xanh' | 'Vàng' | 'Hồng' | 'Tím' | 'Xanh lá';
type AoDaiPattern = 'Trơn' | 'Hoa văn' | 'Truyền thống' | 'Hiện đại' | 'Thêu rồng phượng';
type AoDaiCollar = 'Cổ cao' | 'Cổ tròn' | 'Cổ thuyền' | 'Cổ chữ V';
type TShirtType = 'Có cổ (Polo)' | 'Không cổ (Cổ tròn)';
type TShirtColor = 'Trắng' | 'Đen' | 'Xanh' | 'Xám' | 'Đỏ' | 'Vàng';
...
export default function App() {
  ...
  const [vestTie, setVestTie] = useState(true);
  const [tieColor, setTieColor] = useState<TieColor>('Đỏ');
  const [aoDaiColor, setAoDaiColor] = useState<AoDaiColor>('Trắng');
  const [aoDaiPattern, setAoDaiPattern] = useState<AoDaiPattern>('Trơn');
  const [aoDaiCollar, setAoDaiCollar] = useState<AoDaiCollar>('Cổ cao');
  const [tShirtType, setTShirtType] = useState<TShirtType>('Có cổ (Polo)');
  const [tShirtColor, setTShirtColor] = useState<TShirtColor>('Trắng');
  ...
  const getAspectRatio = (size: PhotoSize) => {
    if (size === '2x3 cm') return 2 / 3;
    if (size === '3x4 cm') return 3 / 4;
    if (size === '4x6 cm') return 4 / 6;
    if (size === 'Passport (3.5x4.5 cm)') return 3.5 / 4.5;
    return 3 / 4;
  };

  const currentAspect = getAspectRatio(photoSize);
  ...
  const generateIDPhoto = async () => {
    ...
    try {
      ...
      const prompt = `
        ...
        - Outfit: ${outfit}. ${
          outfit === 'Áo Vest' 
            ? `Style: ${vestStyle}, Color: ${vestColor}, ${gender === 'Nữ' ? 'Long sleeves (no short sleeves),' : ''} ${vestTie ? `with ${tieColor} tie` : 'no tie'}.` 
            : outfit === 'Áo Dài'
            ? `Color: ${aoDaiColor}, Pattern: ${aoDaiPattern}, Collar: ${aoDaiCollar}.`
            : outfit === 'Áo Thun'
            ? `Type: ${tShirtType}, Color: ${tShirtColor}.`
            : outfit === 'Áo Đoàn'
            ? `Vietnamese Youth Union Shirt (Áo Đoàn Thanh Niên), blue color, with logo on left arm.`
            : ''
        }
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <button 
                        onClick={() => {
                          ...
                          setTieColor('Đỏ');
                          setAoDaiColor('Trắng');
                          setAoDaiPattern('Trơn');
                          setAoDaiCollar('Cổ cao');
                          setTShirtType('Có cổ (Polo)');
                          setTShirtColor('Trắng');
            ...
            {activeTab === 'outfit' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
                <ControlGroup label="Loại trang phục" icon={Shirt}>
                  <div className="grid grid-cols-2 gap-2">
                    {['Áo sơ mi', 'Áo Vest', 'Áo Dài', 'Áo Thun', 'Áo Đoàn', 'Đồ Công Sở'].map(o => (
                  ...
                  <div className="p-5 bg-blue-50/50 rounded-3xl border border-blue-100 space-y-5">
                    ...
                    <ControlGroup label="Màu Vest">
                      <div className="flex flex-wrap gap-3">
                        {['Đen', 'Xanh Navy', 'Xám', 'Trắng', 'Đỏ đô', 'Be'].map(c => (
                          <button 
                            ...
                            style={{ backgroundColor: c === 'Đen' ? '#000' : c === 'Xanh Navy' ? '#000080' : c === 'Xám' ? '#808080' : c === 'Đỏ đô' ? '#800000' : c === 'Be' ? '#F5F5DC' : '#fff' }}
                            title={c}
                          />
                        ))}
                      </div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-white/50 rounded-xl border border-blue-100">
                      <span className="text-xs font-bold text-slate-600">Có cà vạt (Tie)</span>
                      <Switch active={vestTie} onClick={() => setVestTie(!vestTie)} />
                    </div>
                    {vestTie && (
                      <ControlGroup label="Màu cà vạt">
                        <div className="grid grid-cols-3 gap-2">
                          {['Đỏ', 'Xanh', 'Đen', 'Vàng', 'Họa tiết'].map(c => (
                            <TabButton key={c} active={tieColor === c} onClick={() => setTieColor(c as TieColor)} small>{c}</TabButton>
                          ))}
                        </div>
                      </ControlGroup>
                    )}
                    {gender === 'Nữ' && (
                      <p className="text-[10px] text-blue-600 font-medium italic">* Mặc định Vest nữ tay dài chuyên nghiệp</p>
                    )}
                  </div>
                )}

                {outfit === 'Áo Dài' && (
                  <div className="p-5 bg-pink-50/50 rounded-3xl border border-pink-100 space-y-5">
                    <ControlGroup label="Màu Áo Dài">
                      <div className="grid grid-cols-3 gap-2">
                        {['Trắng', 'Đỏ', 'Xanh', 'Vàng', 'Hồng', 'Tím', 'Xanh lá'].map(c => (
                          <TabButton key={c} active={aoDaiColor === c} onClick={() => setAoDaiColor(c as AoDaiColor)} small>{c}</TabButton>
                        ))}
                      </div>
                    </ControlGroup>
                    <ControlGroup label="Họa tiết">
                      <div className="grid grid-cols-2 gap-2">
                        {['Trơn', 'Hoa văn', 'Truyền thống', 'Hiện đại', 'Thêu rồng phượng'].map(p => (
                          <TabButton key={p} active={aoDaiPattern === p} onClick={() => setAoDaiPattern(p as AoDaiPattern)} small>{p}</TabButton>
                        ))}
                      </div>
                    </ControlGroup>
                    <ControlGroup label="Kiểu cổ áo">
                      <div className="grid grid-cols-2 gap-2">
                        {['Cổ cao', 'Cổ tròn', 'Cổ thuyền', 'Cổ chữ V'].map(c => (
                          <TabButton key={c} active={aoDaiCollar === c} onClick={() => setAoDaiCollar(c as AoDaiCollar)} small>{c}</TabButton>
                        ))}
                      </div>
                    </ControlGroup>
                  </div>
                ...
                {outfit === 'Áo Thun' && (
                  <div className="p-5 bg-green-50/50 rounded-3xl border border-green-100 space-y-5">
                    <ControlGroup label="Kiểu áo thun">
                      <div className="grid grid-cols-2 gap-2">
                        {['Có cổ (Polo)', 'Không cổ (Cổ tròn)'].map(t => (
                          <TabButton key={t} active={tShirtType === t} onClick={() => setTShirtType(t as TShirtType)} small>{t}</TabButton>
                        ))}
                      </div>
                    </ControlGroup>
                    <ControlGroup label="Màu sắc">
                      <div className="grid grid-cols-3 gap-2">
                        {['Trắng', 'Đen', 'Xanh', 'Xám', 'Đỏ', 'Vàng'].map(c => (
                          <TabButton key={c} active={tShirtColor === c} onClick={() => setTShirtColor(c as TShirtColor)} small>{c}</TabButton>
                        ))}
                      </div>
                    </ControlGroup>
                  </div>
                ...
                {outfit === 'Áo Đoàn' && (
                  <div className="p-5 bg-blue-50/50 rounded-3xl border border-blue-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-blue-900">Áo Đoàn Thanh Niên</p>
                        <p className="text-[10px] text-blue-600">Trang phục truyền thống Đoàn TNCS Hồ Chí Minh</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className={`relative w-full flex items-center justify-center gap-4 transition-all duration-500 ${compareMode ? 'flex-row' : 'flex-col'}`}>
              ...
              {compareMode && (croppedImage || originalImage) && (
                <motion.div 
                  ...
                  className="relative w-full max-w-[400px] bg-white rounded-[32px] shadow-xl overflow-hidden flex items-center justify-center border-[12px] border-white"
                  style={{ aspectRatio: currentAspect }}
                >
                  <img src={croppedImage || originalImage || ''} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              ...
              <div className={`relative w-full transition-all duration-500 bg-white rounded-[40px] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] overflow-hidden flex items-center justify-center border-[16px] border-white ${
                compareMode ? 'max-w-[400px]' : 'max-w-[480px]'
              }`} style={{ aspectRatio: currentAspect }}>
                <AnimatePresence mode="wait">
                  {showOriginal && originalImage && !compareMode ? (
                    <motion.img 
                      key="original-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                      src={originalImage} className="w-full h-full object-cover z-30 absolute inset-0 bg-white" referrerPolicy="no-referrer"
                    />
                  ) : null}
                  ...
                  {resultImage ? (
                    <motion.img 
                      ...
                      src={resultImage} className="w-full h-full object-cover" referrerPolicy="no-referrer"
                    />
